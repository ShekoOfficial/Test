...
exports.run = async (bot, message, args) => {
  ...
  if(!args[0]) return message.reply("**Mention a user or users that you want to bond.** `?bond <user> <user>`")
   ...
   let user3 = message.guild.member(message.guild.members.get(args[2]));
    ...
    if(!args[1]){
        ...
        .setColor("#f5a3fa")
        .addField("Users", `${message.author} and ${args[0]}`)
    ...
    if(!args[2]){
        ...
        .setColor("#f5a3fa")
        .addField("Users", `${args[0]} and ${args[1]}`)
        .addField("Bond Score", `${bondLevel}%`)
        .addField("Bond Bar", bondLevelResults)
        .addField("Summary", ship);
  ...
  if(!args[3]) {
        ...
        .setColor("#f5a3fa")
        .addField("Users", `${args[0]} x ${args[1]} and ${args[2]}`)
        .addField("Bond Score", `${bondLevel}%`)
        .addField("Bond Bar", bondLevelResults)
        .addField("Summary", ship);
