...
module.exports.run = async (client, msg, args) => {
  try {
    ...
    if (!voiceChannel) return msg.reply("You must be in the join voice channel, to start music!");
    if (client.listenMOE.has(msg.guild.id)) return msg.reply("Woop im currently play listen.moe. maybe you can ~~wait~~ or stop it!");
    if (client.queue.has(msg.guild.id) && voiceChannel.id !== client.queue.get(msg.guild.id).voiceChannel.id) return msg.reply(`> You must be in **${client.queue.get(msg.guild.id).voiceChannel.name}** to play music!`);
    const permissions = voiceChannel.permissionsFor(msg.client.user);
    if (!permissions.has("CONNECT")) return msg.reply("🚫 I don't have permissions **CONNECT**");
    if (!permissions.has("SPEAK")) return msg.reply("🚫 I don't have permissions **SPEAK**");
    ...
    try {
      ...
      try {
        ...
        let m = await msg.channel.send(`${msg.author} 🏴 Loading song...`);
...
function embed(msg, song, type = "biasa") {
  ...
  return msg.send(`✅ **Now Playing :**__**${song.title}**__ - [${song.author}]`);
}

module.exports.conf = {
  aliases: ["ps", "playsearch"],
  clientPerm: "",
  authorPerm: ""
};
