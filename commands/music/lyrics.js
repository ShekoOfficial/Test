...
module.exports.run = async (client, msg, args) => {
  ...
  try {
    ...
    if (!body.response.hits.length)
      return msg.channel.send("🚫 No result found");
    const result = body.response.hits.splice(0, 5);
    const thisMess = await msg.channel.send(
      embed.setDescription(
        result
          .map(
            (x, i) => `${number[i]}[${x.result.full_title}](${x.result.url})`
          )
          .join("\n")
      )
    );
    ...
    const filter = (rect, usr) =>
      number.includes(rect.emoji.name) && usr.id === msg.author.id;
